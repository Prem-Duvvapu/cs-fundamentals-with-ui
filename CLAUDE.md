...
- `utils/topicProgress.js` + `hooks/useTopicProgress.js` — per-topic bookmark/completed state, persisted to `localStorage` and broadcast via a `window` `CustomEvent`, same pattern as `hooks/useTheme.js`. Purely client-side; not part of topic registration and has no backend counterpart. `HomePage.jsx` and `TopicPage.jsx` both render bookmark/complete toggles through this hook.
