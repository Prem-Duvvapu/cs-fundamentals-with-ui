...
- `components/shared/ProductTour.jsx` + `hooks/useProductTour.js` — a guided, spotlight-and-tooltip overlay walking through `utils/tourSteps.js`'s ordered steps (home page, then one topic page). Both are mounted once in `App.jsx`, **a sibling of `<Routes>`, not inside a page** — that's what lets the tour's own step-driven `navigate()` calls survive the route change instead of unmounting mid-tour. `utils/tourPosition.js` is the pure placement function (viewport-clamped, flips above/below); a step's target is looked up with a bounded `requestAnimationFrame` retry that degrades to a centered tooltip rather than hanging if the element never mounts. First-visit auto-show is a `localStorage` flag (same convention as theme/progress) **and is guarded to `location.pathname === '/'`**: without that guard the tour started on any route and its first step's `path: '/'` immediately navigated a first-time visitor off whatever deep link they opened, with the back button unable to recover it. `AppRouting.test.jsx` locks this down — deep links must survive a first visit, and the tour must still auto-open on `/`.
