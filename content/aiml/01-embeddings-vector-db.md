...
The Transformer creates contextual token representations through **self-attention**. Each token
projects query, key, and value vectors; attention weights let its representation incorporate the
other tokens that matter in this sequence. That is why “bank” moves to a finance neighborhood in
“bank approved the loan” but to a geography neighborhood in “sat on the river bank.”

Retrieval needs one fixed-size vector rather than one vector per token. An embedding model
therefore pools token representations—commonly mean pooling or a trained special token—and often
normalizes the result. Query and document encoders are trained so relevant pairs occupy nearby
positions in the same space; self-attention supplies context, while the retrieval objective
teaches which kinds of meaning should count as near.
...
### Vector retrieval, chunking, and reranking

A production retrieval pipeline separates inexpensive broad recall from expensive precise ranking.
The same versioned embedding model must encode indexed chunks and live queries; vectors from
different models do not share a meaningful coordinate system.

```mermaid
flowchart LR
    Doc["Source document"] --> Split["Structure-aware chunking"]
    Split --> Embed["Document embeddings"]
    Embed --> Index["ANN vector index"]
    Query["User query"] --> QEmbed["Query embedding"]
    QEmbed --> Retrieve["Retrieve top 100 candidates"]
    Index --> Retrieve
    Retrieve --> Filter["Apply metadata and access filters"]
    Filter --> Rerank["Cross-encoder reranks top 30"]
    Rerank --> Context["Return best 5 chunks"]
```

The online path normally has four stages:

1. **Embed the query** using the query instruction and model version used during indexing.
2. **Retrieve candidates** with cosine, dot-product, or Euclidean ANN search.
   Candidate count is intentionally larger than the final result count to protect recall.
3. **Filter and fuse** metadata-safe vector results with lexical results when exact names,
   identifiers, or rare terms matter.
4. **Rerank** the reduced set with a more accurate model, then pass only the highest-value chunks
   to the search result page or LLM context window.

Chunking determines the unit the index can retrieve. A perfect ANN index cannot return a fact that
was split away from its required context or buried inside an oversized chunk:

| Chunking choice | Benefit | Failure mode | Typical use |
|---|---|---|---|
| Fixed token window | Simple, predictable batches | Splits headings, tables, or code | Uniform plain text |
| Sliding window with overlap | Preserves facts near boundaries | Duplicates storage and results | Long prose |
| Structure-aware | Keeps sections and paragraphs intact | Produces variable sizes | Documentation and Markdown |
| Semantic boundary | Groups coherent sentences | Extra model cost and instability | Mixed narrative corpora |
| Parent-child | Retrieves small child, returns larger parent | More IDs and joins to manage | Manuals with nested sections |

A useful starting point for prose is 300–600 tokens with 10–20% overlap, but it is a hypothesis,
not a universal constant. Code should respect functions and classes, tables should retain headers,
and question-answer material often works best as one logical pair. Store source ID, section path,
token offsets, model version, and access-control metadata beside each chunk so results can be
traced, filtered, and reindexed.

Overlap rescues information crossing a boundary, but excessive overlap produces near-duplicate
candidates, increases embedding and storage cost, and wastes an LLM's context window. Deduplicate
by source and neighboring offsets before final context assembly. Evaluate chunking with labelled
queries that require both local facts and cross-paragraph context.

Embedding retrieval is usually a **bi-encoder** operation: query and chunk are encoded separately,
so chunk vectors can be precomputed and millions can be searched quickly. A **cross-encoder
reranker** reads the query and each candidate together, allowing token-level attention between
them. That interaction improves ordering but cannot be precomputed, so it should process tens or
hundreds of candidates rather than the full corpus.

For example, retrieve 100 ANN candidates, enforce authorization, fuse lexical matches, rerank the
best 30 in one batch, and return 5. If a reranker takes 4 ms per pair sequentially, 100 pairs would
cost roughly 400 ms before network overhead; reducing to 30 and batching is the difference between
an offline-quality model and a usable request path. Reranking improves precision only among
retrieved candidates—it cannot recover a relevant chunk that candidate retrieval omitted.
...
Cosine similarity requires computing the dot product and dividing it by both vector norms, which adds square roots and division. If embeddings are normalized to unit length on insertion, the denominator is always $1.0$, so SIMD hardware can execute only multiply-accumulate instructions. The optimization is valid only when query and stored vectors use the same normalization policy; otherwise dot product changes the ranking.
...
Scalar Quantization maps each float coordinate to a smaller scalar such as `uint8`, commonly giving a $4\times$ reduction with modest error. Product Quantization replaces whole sub-vectors with learned centroid IDs, achieving much greater compression at higher training cost and recall loss. Benchmark both against an exact index because dataset geometry determines whether the saved RAM justifies the ranking error.

**Q11. Scenario: Retrieval finds relevant sentences, but answers miss qualifications in adjacent paragraphs. What do you change?** `[hard]`
The chunks are likely too small or split without respecting document structure, so the retrieved fact lacks its conditions. Compare larger or parent-child chunks, preserve headings, and add modest overlap while measuring retrieval recall and grounded-answer quality. Do not merely increase overlap everywhere because duplicates consume index storage, reranker capacity, and the LLM context window.
